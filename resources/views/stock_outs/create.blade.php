...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-2xl overflow-hidden">
    ...
    <form action="{{ route('stock-outs.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="mb-6 p-4 bg-amber-50/80 border border-amber-200 rounded-xl text-sm text-amber-800 flex items-start gap-2.5 shadow-sm">
            ...
            <div>
                <strong class="font-bold">Perhatian:</strong> Pengajuan pengeluaran stok ini memerlukan persetujuan berjenjang dari <strong>Superadmin</strong> (Persetujuan Level 1) dan <strong>Pimpinan</strong> (Persetujuan Final) sebelum stok barang dikurangi secara riil.
            </div>
        </div>
        ...
        <div class="space-y-5">
            <div>
                ...
                <select name="item_id" required class="block w-full rounded-lg border-gray-200 shadow-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 sm:text-sm transition-colors duration-150">
                    ...
                    <option value="{{ $item->id }}" {{ old('item_id') == $item->id ? 'selected' : '' }}>{{ $item->nama_barang }} (Stok tersedia: {{ $item->stok }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Jumlah Keluar <span class="text-red-500">*</span></label>
                    <input type="number" name="jumlah" min="1" required placeholder="Contoh: 5" value="{{ old('jumlah') }}" class="block w-full rounded-lg border-gray-200 shadow-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 sm:text-sm transition-colors duration-150">
