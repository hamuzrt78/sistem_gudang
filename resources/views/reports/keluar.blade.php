...
@section('header', 'Laporan Barang Keluar')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-teal-50 to-white">
        <div>
            ...
            <p class="text-xs text-gray-500 font-medium">Rekapitulasi data log barang keluar yang disetujui</p>
        </div>
        <div class="flex items-center gap-2.5">
            <a href="{{ route('laporan.export.excel', ['type' => 'keluar']) }}" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 py-2 rounded-lg shadow-sm hover:shadow transition-all duration-200 transform hover:-translate-y-0.5 text-xs inline-flex items-center gap-1.5">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Export Excel
            </a>
            <a href="{{ route('laporan.export.pdf', ['type' => 'keluar']) }}" class="bg-rose-600 hover:bg-rose-700 text-white font-semibold px-4 py-2 rounded-lg shadow-sm hover:shadow transition-all duration-200 transform hover:-translate-y-0.5 text-xs inline-flex items-center gap-1.5">
    ...
    <div class="p-4 border-b border-gray-100 bg-gray-50/50 flex items-center">
        <form action="{{ route('laporan.keluar') }}" method="GET" class="flex flex-wrap items-center gap-3 w-full">
            ...
            @if(request('start_date') || request('end_date'))
                <a href="{{ route('laporan.keluar') }}" class="text-gray-500 hover:text-gray-700 font-semibold text-sm transition-colors duration-150 ml-2">
                    Reset
                </a>
            @endif
        </form>
        ...
        <table class="min-w-full divide-y divide-gray-100">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-teal-50/20 even:bg-gray-50/30 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-rose-600">-{{ $out->jumlah }}</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate font-medium">{{ $out->keterangan ?: '-' }}</td>
                ...
                <tr><td colspan="8" class="px-6 py-12 text-center text-gray-400">
                    ...
                    <p class="font-medium text-gray-500">Tidak ada data barang keluar.</p>
                </td></tr>
                @endforelse
            </tbody>
        </table>
