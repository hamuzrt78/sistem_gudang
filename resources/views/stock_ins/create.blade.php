...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-2xl overflow-hidden">
    ...
    <form action="{{ route('stock-ins.store') }}" method="POST" class="p-6" id="stockInForm" onsubmit="confirmSubmit(event)">
        @csrf
...
<script>
    let itemIndex = 1;
    const itemOptions = `@foreach($items as $item)<option value="{{ $item->id }}">{{ str_replace("'", "\'", $item->nama_barang) }} (Stok: {{ $item->stok }})</option>@endforeach`;
